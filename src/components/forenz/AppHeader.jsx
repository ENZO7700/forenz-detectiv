...
export default function AppHeader({
  ...
  return (
    <header className="shrink-0 border-b border-slate-800/80 bg-slate-950/85 backdrop-blur-md sticky top-0 z-30 transition-colors">
      {/* Shared Case Banner */}
      {sharedBy && (
        <div className="flex items-center justify-between px-4 py-1.5 bg-violet-950/80 border-b border-violet-800/60 text-violet-200 text-xs">
          <div className="flex items-center gap-2">
            ...
            <span>Zdieľaný vyšetrovací spis od <strong>{sharedBy}</strong> · len na čítanie</span>
          </div>
          <span className="text-[10px] uppercase tracking-wider px-2 py-0.5 rounded bg-violet-900/60 border border-violet-700/50">Read-Only</span>
        </div>
      )}

      {/* Main Bar */}
      <div className="max-w-[1920px] mx-auto px-3 sm:px-4 py-2.5 flex items-center justify-between gap-2 sm:gap-4">
        
        {/* ========================================================================= */}
        {/* 1. BRAND & IDENTITY ISLAND */}
        {/* ========================================================================= */}
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          <button
            ...
            className="lg:hidden p-2 rounded-xl text-slate-400 hover:text-white hover:bg-slate-800/80 transition-colors focus:outline-none focus:ring-2 focus:ring-amber-500/50"
          ...
          {/* Plan Badge (FREE / PRO / AGENCY) with Glow */}
          <button
            type="button"
            onClick={onOpenPricing}
            className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-xl text-[11px] font-bold tracking-wide border shadow-sm transition-all hover:scale-[1.02] ${
              plan === 'agency'
        ...
        {/* ========================================================================= */}
        {/* 2. SEARCH & CASE METRICS ISLAND (Desktop / Tablet) */}
        {/* ========================================================================= */}
        <div className="hidden lg:flex items-center gap-2 flex-1 max-w-md mx-2">
          ...
          <button
            ...
            className="w-full flex items-center justify-between px-3 py-1.5 rounded-xl bg-slate-900/90 hover:bg-slate-800 text-slate-400 hover:text-slate-200 border border-slate-800 hover:border-slate-700 text-xs shadow-inner transition-all group"
        ...
        {/* ========================================================================= */}
        {/* 3. FORENSIC TOOLS & ACTIONS ISLAND */}
        {/* ========================================================================= */}
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          ...
          {/* Desktop Direct Tool Buttons (XL Displays) */}
          <div className="hidden xl:flex items-center gap-1.5">
            {/* Guide */}
            <button
              type="button"
              onClick={onOpenIntro}
              className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-slate-900/80 hover:bg-slate-800 text-slate-300 hover:text-white border border-slate-800 text-xs font-medium transition-colors"
          ...
          {/* Tablet Hybrid Dropdown ("Nástroje ▾" for MD/LG screens) */}
          <div className="hidden md:flex xl:hidden relative" ref={dropdownRef}>
            <button
              type="button"
              onClick={() => setToolsDropdownOpen((prev) => !prev)}
              className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-slate-900/80 hover:bg-slate-800 text-slate-300 border border-slate-800 text-xs font-medium transition-colors"
            >
            ...
            {toolsDropdownOpen && (
              <div className="absolute right-0 top-full mt-1.5 w-48 rounded-2xl bg-slate-900 border border-slate-800 shadow-2xl p-1.5 z-50 flex flex-col gap-0.5 animate-in fade-in zoom-in-95 duration-150">
                <button
                  ...
                  className="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs text-slate-300 hover:text-white hover:bg-slate-800/80 text-left"
                >
          ...
          {!readOnly && (
            <div className="flex items-center gap-1">
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-blue-600/20 hover:bg-blue-600/30 text-blue-300 border border-blue-500/40 text-xs font-semibold shadow-sm transition-all disabled:opacity-40 disabled:pointer-events-none"
              ...
              {activeShare && (
                <button
                  ...
                  className="p-1.5 rounded-xl bg-red-950/60 text-red-300 border border-red-800/60 hover:bg-red-900/60 transition-colors"
          ...
          <div className="relative">
            ...
            >
              ...
              {alertCount > 0 && (
                <span className="absolute 1.5 top-1 right-1 w-3.5 h-3.5 rounded-full bg-red-500 text-[8px] font-bold text-white flex items-center justify-center animate-pulse">
          ...
          {!readOnly && onExport && (
            <button
              ...
              className="hidden 2xl:inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-slate-900/80 hover:bg-slate-800 text-slate-300 border border-slate-800 disabled:opacity-40 text-xs font-medium transition-colors"
          ...
          {!readOnly && onClearCase && documents.length > 0 && (
            <button
              type="button"
              onClick={onClearCase}
              className="hidden xl:inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-slate-900/80 hover:bg-slate-800 text-slate-400 hover:text-slate-200 border border-slate-800 text-xs font-medium transition-colors"
