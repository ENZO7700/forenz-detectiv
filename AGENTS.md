...
- **CI gate = `npm test && npm run lint && npm run typecheck && npm run build`** (per `README.md`). All four pass in this environment (22 integrity/engine tests). This gate does NOT include `test:vitest` or `test:e2e`.
- **`npm run test:vitest` is currently broken regardless of environment:** its `test.include` in `vite.config.js` globs the many `node:test`-based files at the `tests/` root, and Vitest 4 cannot bundle the `node:test` builtin (`Cannot bundle built-in module "node:test"`). Only `tests/components/*.test.jsx` are real Vitest tests. This is a repo-level config issue, not a setup problem.
...
- **Corrupted-storage crash caveat:** a failed upload can leave a bad record in IndexedDB (`ForenzDetectiv_OfflineDB`) that makes the "Alibi & Mapa" and "Časová os" tabs render `Modul zlyhal` (React `useState`-null) on later loads. This is not a fresh-load bug — clear site storage (DevTools → Application → Clear site data, or a fresh incognito window) and all tabs render correctly again.
